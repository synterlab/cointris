...
import { COLS, ROWS, COIN_COLORS, THEMES, type ThemeKey } from './game/constants';
import {
...
export default function App() {
  ...
  useEffect(() => {
    if (['menu','stats','themes','howto','about'].includes(screen)) {
    ...
    if (cleared > 0) {
      ...
      if (canvas) {
        ...
        msgs.forEach(([text,color,dy]) =>
          ref.floats.push({ x:fx, y:l.by+9*l.cs-dy, text, color, vy:-.7, life:1.4, maxLife:1.4 })
        );
      }
  ...
  const gameLoop = useCallback((now: number, ref: G, setScr: typeof setScreen) => {
    ...
    drawSidePanels(ctx, l, ref.score, ref.hiScore, ref.level, ref.lines, ref.combo,
      ref.next.map(n => n.type), ref.held, ref.canHold, tc);
  ...
  const doHold = useCallback(() => {
    ...
    if (prev) {
      ref.current = spawnPiece(prev as any);
    } else {
      const nextType = ref.next[0]?.type || 'I';
      const { piece: bagPiece, newBag } = getNextPiece(ref.bag as any);
      ref.bag = newBag;
  ...
  return (
    <div className="root-wrap" onTouchStart={onTS} onTouchEnd={onTE}>
      ...
      {/* ── MENU / Sub-screens ── */}
      {(['menu','stats','themes','howto','about'] as Screen[]).includes(screen) && (
        <div className="overlay">
          ...
          {screen === 'menu' && (
            <div className="menu-body">
              ...
              <div className="coin-showcase">
                {COIN_DATA.map(c => {
                  const coinBg = [
                    `radial-gradient(circle at 35% 30%, ${c.hi} 0%, ${c.base} 45%, ${c.sh} 100%)`
                  ].join(',');
                  return (
                    <div key={c.type} className="coin-item">
                      <div className="coin-ball" style={{ background: coinBg }}>
          ...
          {screen === 'howto' && (
            <div className="modal-wrap"><div className="modal-card">
              ...
              <div className="modal-subtitle">controls</div>
              ...
              <div className="htp-section">
                <div className="htp-heading">Mobile Swipe</div>
                <div className="htp-grid">
                ...
                <div className="htp-coin-grid">
                  {COIN_DATA.map(c=>(
                    <div key={c.type} className="htp-coin">
                      <div className="htp-coin-dot" style={{background:`radial-gradient(circle at 35% 30%,${c.hi},${c.base} 50%,${c.sh})`}}>{c.label}</div>
              ...
              <div className="htp-section">
                ...
                <table className="score-table"><tbody>
                  {[['1 Line','100 × Lv'],['2 Lines (Double)','300 × Lv'],['3 Lines (Triple)','500 × Lv'],['4 Lines (COINTRIS!)','800 × Lv'],['Hard Drop','2 per cell'],['Combo','50 × combo']].map(([k,v])=>(
                  ...
                  return (
                    <div key={key} className={`theme-row${isActive?' active':''}${isLocked?' locked':''}`}
                      ...
                      <div className="theme-dots">
                        {dots.map((c,i) => <div key={i} className="theme-dot" style={{background:isLocked?'#1e1c2e':c}}/>)}
                      </div>
      ...
      {screen === 'gameover' && g.current && (
        <div className="gameover-overlay">
          {Array.from({length:8},(_,i)=>(
            <div key={i} style={{
              position:'absolute', left:`${6+i*12}%`, bottom:'-16px',
              ...
              opacity:.35, animation:`floatUp ${2.5+i*.4}s ${i*.25}s linear infinite`,
              pointerEvents:'none',
            }}/>
          ))}
          <div style={{position:'relative',zIndex:2,width:'100%',maxWidth:340}}>
            ...
            <div className="gameover-grid">
              {[
                ...
                {label:'COMBO', val:`x${g.current.combo}`,          color:'#ff6bff'},
                {label:'BEST',  val:g.current.hiScore.toLocaleString(), color:'#ffd84d'},
              ].map(({label,val,color})=>(
